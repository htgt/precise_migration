...

{
    ...
    GetOptions(
        ...
        'commit'  => \my $commit,
        'transcript_id:s' => \my $transcript_id,
    ) or pod2usage(2);
    ...
    $schema->txn_do(            
        sub {
            for my $design_id (@design_ids) {
                ...
                try {
                    update_design_phase( $schema, $design_id , $transcript_id);
                }
                catch {
                    ERROR($_);
                };
