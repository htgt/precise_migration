...
use HTGT::Utils::TargRep::Update::MirKOGenbank;
...
my $mirko_genbank = HTGT::Utils::TargRep::Update::MirKOGenbank->new( @config );

$mirko_genbank->update_mirko_genbank_files;

__END__

=head1 NAME

update_mirko_genbank_files.pl - Driver script for HTGT::Utils::TargRep::Update::MirKOGenbank

=head1 SYNOPSIS

update_mirko_genbank_files.pl [options]

      --help             Display a brief help message
      --man              Display the manual page
      --debug            Print debug messages
      --warn             Only print warn or higher log messages
      --project=s        Optionally specify mirko project ids to work on
      --commit           Run script in commit mode, making changes to the targ rep
      --check_genbank    Check and update existing mirko genbank files
      --eng_seq_config=s Specify a EngSeq config file

=head1 DESCRIPTION

Update the mirKO Genbank files in the Targ Rep.
