...
sub list_seqs {
    ...
    if ( $params{type} ) {
        $rs = $self->_schema->resultset( 'EngSeq' )->search(
...
sub _fetch_seq {
    ...
    if ( $count == 1 ) {
        ...
        EngSeqBuilder::Exception->throw( sprintf "Found no sequences with name '%s' of type '%s'", $name, $type ? $type : 'ANY' );
    }
}   
...
sub fetch_seq {
    ...
    return $self->_mk_bio_seq(
        $self->_fetch_seq( $params{name}, $params{type} ),
        \%params
    );
}
...
sub conditional_vector_seq {
    ...
    my %params = validated_hash( \@_, $self->param_spec( qw( chromosome
                                                             strand
                                                             backbone
                                                             five_arm_start
                                                             five_arm_end
...
sub _get_allele_five_arm_seq {
    ...
    my ($five_arm_feat_start, $five_arm_feat_end ) = $self->_slice_rel_coord(
        ...
        $params->{strand},    $params->{five_arm_start}, $params->{five_arm_end}
    );
...
sub _get_allele_three_arm_seq {
    ...
    my ($three_arm_feat_start, $three_arm_feat_end ) = $self->_slice_rel_coord(
        $flank_genomic_start, $flank_genomic_end,
        $params->{strand},    $params->{three_arm_start}, $params->{three_arm_end}
    );
...
sub _slice_rel_coord {
    my ( $self, $slice_genomic_start, $slice_genomic_end, $strand, $genomic_coord_start,
        $genomic_coord_end ) = @_;
...
sub _check_seq_length {
    ...
    if ( $length > $self->max_vector_seq_length ) {
        EngSeqBuilder::Exception->throw( sprintf 'Sequence length (%d) exceeds maximum permitted (%d)', $length, $self->max_vector_seq_length );
    }
}
...
sub _mk_bio_seq {
    ...
    if ( $eng_seq->class eq 'simple' ) {
        $bio_seq = Bio::Seq->new( -alphabet => 'dna',
