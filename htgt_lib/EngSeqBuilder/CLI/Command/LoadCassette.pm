...
sub create_cassette_seq {
    ...
    my $whole_seq_feature = Bio::SeqFeature::Generic->new( -primary => 'misc_feature', tag => { note => 'Synthetic Cassette'} );
    
    $self->eng_seq_builder->create_compound_seq(
        name              => $self->name,
        type              => $self->stage . '-cassette',
        components        => [ $self->append . '_B1_append', $R1_or_B1, $cassette_name, $R2_or_B2, $self->append . '_B2_append' ],
